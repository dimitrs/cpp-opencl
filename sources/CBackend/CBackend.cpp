...
namespace {
  ...
  public:
    static char ID;
    explicit CWriter(formatted_raw_ostream &o, const TargetMachine* TM)
      : FunctionPass(ID), Out(o), TM(TM), IL(0), Mang(0), LI(0),
        TheModule(0), TAsm(0), MRI(0), MOFI(0), TCtx(0), TD(0),
        OpaqueCounter(0), NextAnonValueNumber(0), isKernel(false) {
...
CWriter::printSimpleType(raw_ostream &Out, Type *Ty, bool isSigned,
  ...
  case Type::IntegerTyID: {
    ...
    if (NumBits == 1)
      ...
      return Out << (isSigned?"":"unsigned ") << "long "<< NameSoFar;
    else {
...
void CWriter::printConstant(Constant *CPV, bool Static) {
  ...
  if (ConstantInt *CI = dyn_cast<ConstantInt>(CPV)) {
    ...
    if (Ty == Type::getInt1Ty(CPV->getContext()))
      ...
      Out << CI->getZExtValue() << "ul";  // DIM, was "ull"
    else {
      Out << "((";
...
static void generateCompilerSpecificCode(formatted_raw_ostream& Out,
  ...
  Out << "#ifdef __GNUC__\n"
      ...
      << "#endif\n\n";
#endif
    Out << "#define LLVM_NAN(NanStr)   ((double)0.0)           /* Double */\n"
        << "#define LLVM_NANF(NanStr)  0.0F                    /* Float */\n"
        << "#define LLVM_NANS(NanStr)  ((double)0.0)           /* Double */\n"
        << "#define LLVM_NANSF(NanStr) 0.0F                    /* Float */\n"
        << "#define LLVM_INF           ((double)0.0)           /* Double */\n"
...
*/
  ...
  // predicates
  Out << "int llvm_fcmp_ord(float X, float Y) { ";
  Out << "return X == X && Y == Y; }\n";
  Out << "int llvm_fcmp_uno(float X, float Y) { ";
  Out << "return X != X || Y != Y; }\n";
  Out << "int llvm_fcmp_ueq(float X, float Y) { ";
  Out << "return X == Y || llvm_fcmp_uno(X, Y); }\n";
  Out << "int llvm_fcmp_une(float X, float Y) { ";
  Out << "return X != Y; }\n";
  Out << "int llvm_fcmp_ult(float X, float Y) { ";
  Out << "return X <  Y || llvm_fcmp_uno(X, Y); }\n";
  Out << "int llvm_fcmp_ugt(float X, float Y) { ";
  Out << "return X >  Y || llvm_fcmp_uno(X, Y); }\n";
  Out << "int llvm_fcmp_ule(float X, float Y) { ";
  Out << "return X <= Y || llvm_fcmp_uno(X, Y); }\n";
  Out << "int llvm_fcmp_uge(float X, float Y) { ";
  Out << "return X >= Y || llvm_fcmp_uno(X, Y); }\n";
  Out << "int llvm_fcmp_oeq(float X, float Y) { ";
  Out << "return X == Y ; }\n";
  Out << "int llvm_fcmp_one(float X, float Y) { ";
  Out << "return X != Y && llvm_fcmp_ord(X, Y); }\n";
  Out << "int llvm_fcmp_olt(float X, float Y) { ";
  Out << "return X <  Y ; }\n";
  Out << "int llvm_fcmp_ogt(float X, float Y) { ";
  Out << "return X >  Y ; }\n";
  Out << "int llvm_fcmp_ole(float X, float Y) { ";
  Out << "return X <= Y ; }\n";
  Out << "int llvm_fcmp_oge(float X, float Y) { ";
  Out << "return X >= Y ; }\n";
...
void CWriter::printFunctionSignature(const Function *F, bool Prototype) {
  ...
  std::string KernelPrefix {"_Kernel_"};
  if (GetValueName(F).substr(0, KernelPrefix.size()) == KernelPrefix) {
      isKernel = true;
      FunctionInnards << " __kernel ";
  }
  ...
  for (; AI != AE; ++AI, ++ArgNo) {
    ...
    if (ArgNo < NumDeclaredParams &&
      ...
      printType(Out, FTy->getParamType(ArgNo),
            /*isSigned=*/!PAL.hasAttribute(ArgNo+1, Attribute::ZExt));
      Out << ')';
    }
